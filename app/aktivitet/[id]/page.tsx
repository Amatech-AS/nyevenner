...
const Map = dynamic(() => import('@/components/Map'), { ssr: false, loading: () => <div style={{height:'200px', background:'#f1f5f9', display:'flex', alignItems:'center', justifyContent:'center', color:'#94a3b8'}}>Laster kart...</div> });

type AktivitetType = { id: string; tittel: string; beskrivelse: string; dato: string; sted: string; postnummer: string; max_deltakere: number | null; image_url: string | null; price: number; creator_id: string; }

// (Sett inn samme Smart Bilde-kode her hvis du vil være 100% sikker på konsistens, 
// men den leser nå image_url fra basen som vi fikset med SQL)
const imageCollections = { default: ['https://images.unsplash.com/photo-1523301343968-63214359d56b?q=80&w=400'] };
const getSmartImage = (t, i) => imageCollections.default[0]; // Enkel fallback
...
export default function AktivitetDetalj() {
  ...
  const slettAktivitet = async () => {
    if (confirm('Er du sikker?')) { await supabase.from('activities').delete().eq('id', id); router.push('/'); }
  }
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '32px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: '1', minWidth: '300px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div style={{ backgroundColor: 'white', padding: '16px', borderRadius: '24px', border: '1px solid #e2e8f0' }}>
                <div style={{ fontWeight: 'bold', marginBottom: '12px', display: 'flex', gap: '8px', alignItems:'center', color:'#334155' }}><div style={{background:'#eff6ff', padding:'8px', borderRadius:'50%'}}><MapPin size={16} color="#2563eb"/></div>
                    {/* FIKS: Viser nå også postnummer */}
