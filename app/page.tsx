...
import { MapPin, Calendar, Search, Users, ArrowRight, Info, CheckCircle, Loader2, Smile, Heart, Clock } from 'lucide-react';
...
// --- HJELPEFUNKSJON FOR Å TOLKE NORSK DATO-TEKST ---
const parseNorwegianDate = (dateStr: string) => {
  // Format: "Lørdag 6. desember kl 18:00"
  const months = {
  ...
  try {
    ...
    return 0; // Hvis feil, legg sist/først
  }
};
...
export default function LandingPage() {
  ...
  // FILTER STATE: 'alle' | 'naer' (3 siffer) | 'by' (2 siffer) | 'dato' (sortering)
  ...
  const fetchData = async () => {
    ...
    // Henter mange aktiviteter så vi har nok å filtrere på client-side
  ...
  let prosessertListe = aktiviteter.filter(a => {
      ...
      if (userPostnummer && a.postnummer) {
          if (activeFilter === 'naer') {
              // Nær meg: Matcher 3 første siffer (Veldig lokalt)
              return a.postnummer.substring(0, 3) === userPostnummer.substring(0, 3);
          }
          if (activeFilter === 'by') {
              // Min by: Matcher 2 første siffer (Hele regionen/byen)
  ...
  if (activeFilter === 'dato') {
      prosessertListe.sort((a, b) => {
          return parseNorwegianDate(a.dato) - parseNorwegianDate(b.dato);
      });
  }
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#F8FAFC', paddingBottom: '80px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto 48px auto' }}>
            ...
            <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '24px', textAlign: 'center', boxShadow: '0 10px 20px -5px rgba(0,0,0,0.03)' }}>
                <h2 style={{ fontSize: '20px', fontWeight: '900', color: '#0f172a', marginBottom: '8px' }}>Relasjoner skapes hele livet</h2>
        ...
        {user && (
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '12px', marginBottom: '40px' }}>
                <button onClick={() => setActiveFilter('alle')} style={getBtnStyle(activeFilter === 'alle')}>
                    Vis alle
                </button>
                <button onClick={() => setActiveFilter('naer')} style={getBtnStyle(activeFilter === 'naer')}>
                    📍 Nær meg (Nabolag)
                </button>
                <button onClick={() => setActiveFilter('by')} style={getBtnStyle(activeFilter === 'by')}>
                    🏙️ Min by (Region)
                </button>
                <button onClick={() => setActiveFilter('dato')} style={getBtnStyle(activeFilter === 'dato')}>
                    📅 Etter dato
                </button>
            </div>
        )}
        ...
        {loading ? <div style={{ textAlign: 'center', padding: '40px' }}><Loader2 className="animate-spin"/></div> : (
          <>
            ...
            {synligeAktiviteter.length === 0 && (
                <div style={{ textAlign:'center', padding:'40px', color:'#64748b' }}>Fant ingen aktiviteter som passet filteret.</div>
            )}
          </>
        )}
      ...
      {/* Flytende knapp for mobil (Ligger nederst til høyre for enkel tilgang) */}
      <div style={{position: 'fixed', bottom: '32px', right: '24px', zIndex: 40}}>
        <Link href="/ny-aktivitet" style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', background: '#10b981', color: 'white', padding: '16px 24px', borderRadius: '99px', fontWeight: 'bold', fontSize: '16px', textDecoration: 'none', boxShadow: '0 15px 30px -5px rgba(16, 185, 129, 0.4)', transition: 'transform 0.2s' }}>
            + Ny
        </Link>
      </div>
    </main>
