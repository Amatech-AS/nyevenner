        ...
        <div style={{ backgroundColor: 'white', padding: '40px', borderRadius: '24px', boxShadow: '0 10px 30px -10px rgba(0,0,0,0.1)', border: '1px solid #e2e8f0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '2px solid #e2e8f0', fontSize: '16px', outline: 'none', fontWeight: '500' }} placeholder="din@epost.no" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', color: '#334155', marginBottom: '8px', paddingLeft: '4px' }}>Passord</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '2px solid #e2e8f0', fontSize: '16px', outline: 'none', fontWeight: '500' }} placeholder="Min. 6 tegn" />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '8px' }}>
                <button onClick={handleLogin} disabled={loading} style={{ width: '100%', padding: '16px', backgroundColor: '#0f172a', color: 'white', borderRadius: '12px', fontWeight: 'bold', fontSize: '16px', border: 'none', cursor: 'pointer', display:'flex', justifyContent:'center', alignItems:'center', gap:'8px' }}>{loading ? <Loader2 className="animate-spin"/> : 'Logg inn med e-post'}</button>
                <button onClick={handleSignUp} disabled={loading} style={{ width: '100%', padding: '16px', backgroundColor: 'white', color: '#0f172a', borderRadius: '12px', fontWeight: 'bold', fontSize: '16px', border: '2px solid #e2e8f0', cursor: 'pointer' }}>Ny bruker? Registrer deg her</button>
            </div>
          </div>
        </div>

        <div style={{ marginTop: '32px', padding: '24px', backgroundColor: '#eff6ff', borderRadius: '20px', border: '1px solid #dbeafe' }}>
          <h3 style={{ fontSize: '16px', fontWeight: 'bold', color: '#1e40af', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}><HelpCircle size={20}/> Trenger du hjelp?</h3>
          <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
             <div style={{ background:'white', padding:'8px', borderRadius:'50%', height:'fit-content' }}><User size={16} color="#2563eb"/></div>
             <div><p style={{ fontWeight:'bold', fontSize:'14px', color:'#1e3a8a', marginBottom:'4px' }}>Innlogging</p><p style={{ fontSize:'14px', color:'#3b82f6', lineHeight:'1.5' }}>Du kan nå bruke din Google eller Facebook konto hvis du har det. Det er det enkleste!</p></div>
          </div>
        </div>
      </div>
    </div>
