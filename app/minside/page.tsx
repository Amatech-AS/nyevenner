...
import { LogOut, Calendar, ArrowRight, History, ArrowLeft, User, Heart, UserMinus, X, AlertTriangle, Trophy, Star, MapPin, Coins } from 'lucide-react';
...
export default function MinSide() {
  ...
  const [stats, setStats] = useState({ denneMnd: 0, totalt: 0, totalKostnad: 0 }); // Ny stat!
    ...
    const load = async () => {
      ...
      if (paameldinger && paameldinger.length > 0) {
        ...
        if (akts) {
          ...
          const totalKostnad = akts.reduce((sum, a) => sum + (a.price || 0), 0); // Summerer pris
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#F8FAFC', paddingBottom: '80px', fontFamily: 'system-ui, sans-serif' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                    <input type="radio" name="reason" value="not_needed" onChange={e => setUnlinkReason(e.target.value)} /> <span>Vi trenger ikke koblingen lenger</span>
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                    <input type="radio" name="reason" value="passed_away" onChange={e => setUnlinkReason(e.target.value)} /> <span>Brukeren har gått bort 🌹</span>
                  </label>
                </div>
              </div>
            )}
      ...
      <div style={{ maxWidth: '800px', margin: '40px auto', padding: '0 20px' }}>
          ...
          <div style={{ position: 'relative', zIndex: 10 }}>
            ...
            <div style={{ marginTop: '32px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, background: '#f0f9ff', padding: '20px', borderRadius: '16px', border: '1px solid #bae6fd' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', color: '#0284c7' }}><Star size={20} fill="#0284c7" /> <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase' }}>Aktiviteter i år</span></div>
        ...
        {/* PÅRØRENDE LOGIKK (Beholder samme som før) */}
        <div style={{ marginBottom: '40px' }}>
                        ...
                        <div style={{ borderTop: '1px solid #fcd34d', paddingTop: '16px', marginTop: '8px' }}>
                            ...
                            {relasjoner.map(rel => (
                                <div key={rel.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'white', padding: '12px', borderRadius: '12px', marginBottom: '8px' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={{ background: '#fef3c7', padding: '8px', borderRadius: '50%' }}><User size={16} color="#d97706"/></div><span style={{ fontWeight: 'bold', color: '#451a03' }}>{rel.full_name}</span></div>
            ...
            {profil?.rolle === 'familie' && (
                <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '20px', padding: '24px' }}>
                    ...
                    {relasjoner.length === 0 ? (<p style={{ color: '#64748b' }}>Du følger ingen enda.</p>) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {relasjoner.map(rel => (
